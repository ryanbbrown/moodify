...
class App extends Component {
    ...
    async createEmpty() {
        ...
        let prom = spotifyApi.createPlaylist(this.state.userId, {
            ...
            public: this.state.playlistType,
        });
        let data = await prom;
        newId = data.id;
    ...
    moodSorting(audioFeatures, mood) {
        if (mood === 'happy') {
            audioFeatures.sort(
                (a, b) => b.valence + b.energy - (a.valence + a.energy)
            );
        } else if (mood === 'sad') {
    ...
    async getSortedTracks(mood, numSongs) {
        console.log(mood);
        ...
        // audioFeatures = audioFeatures.slice(0, numSongs);
        let finalArr = [];
        for (var object of audioFeatures) {
            finalArr.push(object.id);
        }
        console.log(finalArr);
        finalArr = [...new Set(finalArr)]; // remove duplicates
        finalArr = finalArr.slice(0, numSongs);
        console.log(finalArr);
        return finalArr;
    }
    ...
    async printAudioFeatures() {
        let x = this.getAudioFeatures(this.state.mood, this.state.numSongs);
        let y = await x;
        console.log(y);
    }
                                                ...
                                                <Col xs={8}>
                                                    ...
                                                    <Field
                                                        ...
                                                        render={({
                                                            ...
                                                            <FormGroup controlId="playlistType">
                                                                <FormControl
                                                                    as="select"
                                                                    onChange={
                                                                        field.onChange
                    ...
                    <Button
                        ...
                        onClick={(e) => {
                            ...
                            window.location.href =
                                'http://moodify-login.herokuapp.com/login';
                        }}
                    >
                        {' '}
                        Login with Spotify
